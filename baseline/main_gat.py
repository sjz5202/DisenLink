from torch_geometric.nn import GCNConv, ARGVA, ARGA,GATConv,GAE
import torch
...
for run in range(args.run):
    ...
    model = VGAE(VariationalGATEncoder(nfeat, args.nhidden,args.nembed,args.head)).to(device)
...
if args.dataset in ['twitch-e','fb100']:
    ...
    filename = f'performance/{args.dataset}_gat1.csv'
else:
    filename = f'performance/{args.dataset}_gat1.csv'
