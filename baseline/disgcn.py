...
class DisenGCN(nn.Module):
    ...
    #
    def __init__(self, nfeat, nclass, split_mlp=False):
        super(DisenGCN, self).__init__()
        ncaps=7
        nhidden=32
        routit=6
        nlayer=5
        self.pca = SparseInputLinear(nfeat, ncaps * nhidden)
        conv_ls = []
        for i in range(nlayer):
            conv = NeibRoutLayer(ncaps, routit)
            self.add_module('conv_%d' % i, conv)
            conv_ls.append(conv)
        self.conv_ls = conv_ls
        if split_mlp:
            self.clf = SplitMLP(nclass, nhidden * ncaps,
                                nclass)
        else:
            self.clf = nn.Linear(nhidden * ncaps, nclass)

    def _dropout(self, x):
        return fn.dropout(x, self.dropout, training=self.training)

    def forward(self, x, src_trg):
        x = fn.leaky_relu(self.pca(x))
        for conv in self.conv_ls:
            x = fn.leaky_relu(conv(x, src_trg))
        x = self.clf(x)
        return x
