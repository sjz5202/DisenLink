...
from model_fea import Disentangle,Factor,Disentangle_2layer
...
for run in range(args.run):
    ...
    if(args.layer==1):
        ...
        model = Disentangle(nfeat, args.hidden, nfactor=args.nfactor, beta=args.beta, t=args.temperature).to(device)
    ...
    if(0):
        ...
        for epoch in range(args.epochs):
            ...
            if(auc>best_auc):
                ...
                torch.save(model.state_dict(), 'saved_model/nembed32.pkl')
            else:
                m+=1
            if(m>200):
                break
    if(1):
        model.load_state_dict(torch.load('saved_model/nembed32.pkl'))
    with torch.no_grad():
        h_all_factor,a_pred,x_pred,Q,h_pooling=model(x,ori_adj)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 1 == 0:
            ...
            acc, nmi, ari, f1 = eva(data.y.cpu().numpy(), q,epoch)
