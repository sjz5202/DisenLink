...
class Disentangle_layer(nn.Module):
    ...
    def forward(self, Z, adj):
        ...
        for i in range(self.nfactor):
            temp = self.beta * Z[i] + (1 - self.beta) * torch.mm(adj, Z[i])
...
class Disentangle(nn.Module):
    def __init__(self, nfeat, nhid,nebed,nfactor,beta,t=1):
        ...
        #self.x_decoders2 = [Dec(4*nebed, nfeat) for _ in range(nfactor)]
        ...
        self.nfactor=nfactor
        self.beta=beta
        self.cluster_layer = Parameter(torch.Tensor(2, nebed))
    ...
    def forward(self,x,adj):
        ...
        #h_pooling=torch.cat(h_all_factor,dim=1)

        #h_pooling=torch.stack([h_all_factor[i]*(alpha_fea.t()[i].repeat(32,1).t()) for i in range(5)],dim=0)
        #h_pooling=torch.sum(h_pooling,dim=0)
        
        #x_pred=self.x_decoders(h_pooling)
        #return torch.cat(h_all_factor,dim=1),link_pred,x_pred,self.get_Q(h_pooling),h_pooling
        return torch.cat(h_all_factor,dim=1),link_pred,1,self.get_Q(h_pooling),h_pooling
        #return h_pooling,link_pred,1,self.get_Q(h_pooling),h_pooling
class Disentangle_fea(nn.Module):
    def __init__(self, nfeat, nhid,nebed,nfactor,beta,nlabel,t=1):
        ...
        self.nlabel=nlabel
        self.nebed=nebed
        self.cluster_layer = Parameter(torch.Tensor(nlabel, nebed))
        torch.nn.init.xavier_normal_(self.cluster_layer.data)
